...
const COACH_MYWAY_RULES: CuratedIntentRule[] = [
  ...
  { knowledgeId: 'privacy-002', pattern: /(?:성적표|모의고사).*(?:사진|결과|점수|보내|공유)/u },
  ...
  { knowledgeId: 'privacy-001', pattern: /(?:민감|개인)\s*정보|보내지\s*말아|채팅.*(?:정보|적어)|비밀번호|주민등록/u },
  ...
  { knowledgeId: 'consultation-006', pattern: /(?:부모|보호자|학생).*(?:같이|함께|동반|혼자).*(?:참석|가야|상담)/u },
  { knowledgeId: 'consultation-005', pattern: /(?:부모|보호자|엄마|아빠).*(?:혼자|만|먼저).*(?:상담|이야기)/u },
  ...
  { knowledgeId: 'consultation-002', pattern: /(?:우리|자녀|아이|학생).*(?:맞|적합|도움).*(?:상담|판단|확인|프로그램)/u },
  { knowledgeId: 'consultation-001', pattern: /(?:처음|코칭|상담).*(?:문의|접수|신청).*(?:방법|채널|어디|순서)|(?:카카오|카톡).*(?:상담|문의)/u },
  ...
  { knowledgeId: 'program-006', pattern: /(?:피드백|진행\s*상황|학습\s*상황|결과).*(?:부모|학부모|전달|공유|알)/u },
  { knowledgeId: 'program-005', pattern: /(?:주\s*몇|일주일|얼마나\s*자주|세션\s*간격|코칭\s*주기|횟수)/u },
  { knowledgeId: 'program-004', pattern: /(?:계획표|플래너|공부\s*계획|학습\s*계획).*(?:짜|설계|관리|점검|같이)/u },
  ...
  { knowledgeId: 'fit-007', pattern: /(?:전과목|전체\s*과목|여러\s*과목|국영수|한\s*과목이\s*아니라|전반적인\s*학습).*(?:관리|모두|한꺼번에|가능|받)/u },
  ...
  { knowledgeId: 'pricing-004', pattern: /(?:카드|계좌이체|결제\s*수단|등록비).*(?:결제|납부|낼|가능)/u },
  ...
  { knowledgeId: 'policy-004', pattern: /(?:등록|코칭|수업).*(?:후|중간).*(?:방식|형태|요일|일정).*(?:변경|바꾸|조정)/u },
