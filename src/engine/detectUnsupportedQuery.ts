...
const GUARD_RULES: GuardRule[] = [
  ...
  },
  {
    ...
    patterns: [
      /(?:이전|지난|다른|타인의?)\s*(?:사용자|학생|고객|학부모)?(?:의|가\s*남긴)?\s*(?:상담|대화|정보|전화번호|학습\s*기록|성적\s*기록|합격\s*학교).*(?:보여|알려|조회|찾아|전달)/u,
...
export function isGuardExplanationFollowUp(query: string): boolean {
  const normalized = normalizeText(query);
  return /^(?:왜\s*)?(?:안|못)\s*(?:되|해|하는).*(?:이유|다음|방법|대안)|왜\s*(?:안|못)\s*(?:돼|되나요|해요)/u.test(normalized);
}
